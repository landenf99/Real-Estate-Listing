...
def normalize_output(data):
    if isinstance(data, dict):
        return {k: normalize_output(v) for k, v in data.items()}
    elif isinstance(data, list):
        return [normalize_output(v) for v in data]
    elif isinstance(data, str):
        return normalize_punctuation(data)
    return data
...
- Never use em dashes or en dashes. Use a standard hyphen, comma, or period instead.
...
def generate_json(prompt: str) -> dict:
    ...
    return normalize_output(json.loads(text))
